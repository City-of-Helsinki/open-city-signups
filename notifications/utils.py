...
def send_notification(user, notification_type, context=None, language=DEFAULT_LANGUAGE):
    logger.debug('Trying to send notification "{}" to {}.'.format(notification_type, user.email))
    ...
    send_mail(subject, html_body, text_body, user.email)
